...
A diferencia del algoritmo de Fuerza Bruta, para no probar con combinaciones de ítems que ya sabemos de antemano que no serán óptimas o que directamente no entrarán en la mochila, haremos podas de factibilidad y de optimalidad, es decir, cuando se cumplan algunas condiciones particulares, no seguiremos decidiendo en la rama en la que estamos, sino que pasaremos a una rama diferente. La idea del pseudocódigo de la función que recorre todo el árbol para encontrar la mejor combinación sería algo así:\par

\begin{algorithm}
\begin{algorithmic}[1]

\Function{recorrer}{$capacidad, tamanoActual, beneficioActual, nuevoItem, mejorBeneficio$}
	\If{\Call{podaPorOptimalidad}{$capacidad, tamanoActual, beneficioActual, nuevoItem, mejorBeneficio$}}
		\If{\Call{podaPorFactibilidad}{$capacidad, tamanoActual, nuevoItem$}}
			\State $beneficioConItem \gets beneficioActual + nuevoItem.beneficio$
			\State $mejorBeneficio \gets max(beneficioConItem, mejorBeneficio)$

			\If{hayItemDespuesDelNuevo}
				\State // siguienteItem es el ítem que viene después del nuevo.
				\State $mejorBeneficio \gets \Call{recorrer}{$capacidad, tamanoActual + nuevoItem.tamano, beneficioConItem, siguienteItem, mejorBeneficio$}$
			\EndIf
		\EndIf
		\If{hayItemDespuesDelNuevo}
			\State $mejorBeneficio \gets \Call{recorrer}{$capacidad, tamanoActual, beneficioActual, siguienteItem, mejorBeneficio$}$
		\EndIf
	\EndIf
...
La función devuelve el mejor beneficio que se puede generar con una combinación de los ítems siguientes sumado al beneficio alcanzado actualmente, o el mejor beneficio anterior, si es que no se superó. Se puede ver que es una función recursiva, y que no se llama a sí misma si es que se debe podar por optimalidad. Tampoco se llama a sí misma agregando el ítem actual si se debe podar por factibilidad. Esta es la forma en la que se eliminan ramas que ya sabemos que no nos servirán para encontrar una mejor solución. Al comienzo del algoritmo, se llama a esta función con los valores iniciales para cada parámetro, y el finalizar el resultado total será lo devuelto por la función.\par
...
\includegraphics{aumentocapacidad15items.png}

Este gráfico se produjo tomando una granuralidad de 4 en la capacidad de la mochila, con 100 muestras de 15 ítems en cada paso, y con cada ítem con un máximo de 5 de tamaño. Como previmos, el tiempo que tarda el algoritmo de Dinámica crece con la capacidad de la mochila, mientras que los otros algoritmos mantienen su desempeño. El que también crece un poco es el de Backtracking, por el hecho de que descarta combinaciones si es que el tamaño supera la capacidad. Al aumentar la capacidad, hay menos combinaciones que se pueden descartar rápidamente. También podemos ver que los tiempos de corrida varían mucho más que con los otros algoritmos.\par
